...
class Manager:
    ...
    def decompose_query(self, query: str) -> list[str]:
        ...
        while not len(tasks_list):
            ...
            try:
                ...
                tasks_list = self.query_decomposer.forward(query=query)
    ...
    def assign_tasks(self, tasks_list: list[str]) -> list[str, dict]:
        ...
        while not len(assigned_tasks):
            ...
            try:
                ...
                assigned_tasks = self.task_assigner.forward(tasks=tasks_list, tools_description=tools_description_text)
            ...
            try:
                ...
                if self.tools[tool_name].tool_type == 'function':
                    ...
                    if self.use_rag:
                        ...
                        new_observation = self.rag_filter.forward(query=task, data=new_observation, max_hits=self.max_rag_hits)

                        new_observation = str(new_observation)

                elif self.tools[tool_name].tool_type == 'llm':

                    new_observation = self.tools[tool_name].forward(query=tool_input, context=observations[-1])
    ...
    def summarize_tasks_outputs(self, tasks_outputs: list[str], context: str='') -> str:
        ...
        summary = self.summarizer.forward(query="\n\n".join(tasks_outputs), context=context)
...
def run_query(query: str, tasks_list: list[str], config_vars: dict) -> str:
    ...
    model_checkpoint = (
        config_vars['TOOLS_SPECIFIC_MODELS'][tool_name] if tool_name in config_vars['TOOLS_SPECIFIC_MODELS'] else
        config_vars['BASE_TEXT_GENERATION_MODEL']
    )
    agent.add_tool(tool_function(model_checkpoint, device_map, max_new_tokens, max_tasks), is_query_decomposer=True)
    ...
    agent.add_tool(tool_function(model_checkpoint, device_map, max_new_tokens), is_task_assigner=True)

    tool_function = rag_filter.rag_filter
    tool_name = tool_function.name
    similarity_fn_name = config_vars['RAG_SIMILARITY_FUNCTION']
    model_checkpoint = (
        config_vars['TOOLS_SPECIFIC_MODELS'][tool_name] if tool_name in config_vars['TOOLS_SPECIFIC_MODELS'] else
        config_vars['BASE_TEXT_EMBEDDING_MODEL']
    )
    agent.add_tool(tool_function(model_checkpoint, device_map, similarity_fn_name), is_rag=True)
    ...
    agent.add_tool(tool_function(model_checkpoint, device_map, max_new_tokens), is_summarizer=True)
    ...
    agent.add_tool(tool_function(model_checkpoint, device_map, max_new_tokens))
