...
class memory_handler:
    ...
    text_embedder: Callable[..., Any]
        embedder class instance
    """

    ### ------------------------------------ ###

    def __init__(self, memory_path: str, text_embedder: Any) -> None:
    ...
    def retrieve_memory(self, context: list[str], max_hits: int=5, score_threshold: float=0.75) -> NDArray:
        ...
        # Retrieve encodings
        select_statement = 'SELECT encoding FROM ai_user_interactions'
        memory_encodings = [self.blob_to_numpy(e[0]) for e in db_cur.execute(select_statement).fetchall()]
        ...
        top_hits = self.text_embedder.compare(context_encoding, memory_encodings, max_hits, score_threshold)
        top_hits_idx = np.unique([t for ts in top_hits.values() for t,s in ts])

        # Retrieve content of memories
        if top_hits_idx.shape[0] > 0:
            select_statement = 'SELECT content FROM ai_user_interactions'
            memory_contents = db_cur.execute(select_statement).fetchall()
            relevant_memories = np.array(memory_contents).ravel()[top_hits_idx]
        else:
            relevant_memories = []
