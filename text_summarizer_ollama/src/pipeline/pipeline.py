...
class Pipeline:
    ...
    class llm_tool:
        ...
        def __init__(self, prompt_path: str) -> None:
            ...
            self.chat = json.loads(raw['chat'].replace('json\n', ''))
            self.output_type = raw['output_type']

        ### -------------------------------- ###

        def forward(self, llm_engine: Callable[..., Any], query: str, think: bool=False) -> str:

            # Update chat
            updated_chat = self.chat.copy()
            updated_chat[1]['content'] = updated_chat[1]['content'].replace('[QUERY]', query)

            # Run LLM
            parsed_llm_output = llm_engine.forward(chat=updated_chat, think=think)
    ...
    def cluster_sentences(self, embeddings: NDArray) -> NDArray:
        ...
        plt.tight_layout()
        plt.savefig('clustering_stats_plot.png', dpi=300)
        plt.close()
    ...
    def plot_clusters_distribution(self) -> None:
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        plt.savefig('clusters_plot.png', bbox_inches='tight', dpi=300)
    ...
    def summarize_text(self) -> None:
        ...
        for cl in unique_clusters:
            ...
            while True:
                try:
                    cl_summary = self.tools['data_summarizer'].forward(llm_engine=self.llm, query=cl_sentences, think=self.think_mode)
            ...
            while True:
                try:
                    cl_topic = self.tools['topic_extractor'].forward(llm_engine=self.llm, query=topics_input, think=self.think_mode)
